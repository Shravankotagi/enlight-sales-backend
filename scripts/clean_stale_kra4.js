...
(async () => {
  ...
  console.log('All KRA4 entries:', kra4?.map(k => k.customer_name));
  ...
  const registered = (customers || []).map(c => c.customer_name.toLowerCase());
  ...
  const toDelete = (kra4 || []).filter(k => {
    ...
    return !registered.some(r => name.includes(r.slice(0, 5)) || r.includes(name.slice(0, 5)));
  });

  console.log('Stale entries to delete:', toDelete.map(t => t.customer_name));

  for (const entry of toDelete) {
    const { error } = await supabase.from('kra_logs').delete().eq('id', entry.id);
    if (error) console.error('Delete error:', entry.customer_name, error.message);
