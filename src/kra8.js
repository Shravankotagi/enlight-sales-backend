...
function fallbackExtractCustomerName(text) {
  ...
  const m1 = clean.match(
    /^([A-Za-z0-9\s.&'-]+?)\s+(?:complaint|issue|problem|rejection|galat|reject)/i,
  );
  ...
  const m2 = clean.match(
    /(?:complaint|issue|problem|rejection)\s+(?:for|from|at|about|of)\s+([A-Za-z0-9\s.&'-]+?)(?:[:,\-]|\s+they|\s+we|\s+material|\s+received|$)/i,
  );
...
async function saveComplaint(details, senderPhone) {
  ...
  try {
    const custName =
      ...
      fallbackExtractCustomerName(details.description) ||
      null;
...
async function checkComplaints() {
  ...
  try {
    ...
    managers.forEach((m) => {
      const teamSalespersonPhones = empList
        .filter((e) => e.manager_id === m.id)
        .map((e) => e.phone)
        .filter(Boolean);
      managerTeamMap[m.phone] = Array.from(
        new Set([m.phone, ...teamSalespersonPhones]),
      );
    });
    ...
    for (const manager of managers) {
      ...
      const teamComplaints = openComplaints.filter((c) =>
        teamPhones.includes(c.reported_by),
      );

      if (teamComplaints.length > 0) {
        const lastReminded =
          notificationThrottleState.managerLastReminded[manager.phone] || 0;
        if (now - lastReminded >= SIX_HOURS_MS) {
          ...
          teamComplaints.forEach((c, idx) => {
            ...
            const hrsOpen = Math.max(
              0,
              Math.round((now - reportedAt.getTime()) / (1000 * 60 * 60)),
            );
            const hoursLeft = Math.max(0, 48 - hrsOpen);
            const repEmp = empList.find((e) => e.phone === c.reported_by);
            const repName = repEmp
              ? repEmp.name
          ...
          console.log(
            `[Complaints Notification] 6-hour team reminder sent to Sales Manager ${manager.name} (${manager.phone}) for ${count} complaints`,
          );
    ...
    if (
      now - lastAdminDigest >= TWENTY_FOUR_HOURS_MS &&
      admins.length > 0 &&
      ...
      for (const admin of admins) {
        ...
        console.log(
          `[Complaints Notification] Daily 24h digest sent to Admin ${admin.name} (${admin.phone})`,
        );
        await new Promise((r) => setTimeout(r, 1000));
      }
    ...
    for (const complaint of openComplaints) {
      ...
      if (hoursElapsed >= 48 && !complaint.escalated) {
        ...
        console.log(
          `[Complaints Notification] Escalation notice sent to salesperson for complaint ${complaint.id}`,
        );
        await new Promise((r) => setTimeout(r, 1000));

        // 24+ hours - Send reminder to salesperson (cooldown 6 hours)
      } else if (hoursElapsed >= 24 && hoursElapsed < 48) {
        const lastComplaintReminded =
        ...
        if (now - lastComplaintReminded >= SIX_HOURS_MS) {
          ...
          notificationThrottleState.complaintLastReminded[complaint.id] = now;
          console.log(
            `[Complaints Notification] 24h reminder sent to salesperson for complaint ${complaint.id}`,
          );
          await new Promise((r) => setTimeout(r, 1000));
        }
      }
  ...
  try {
    ...
    if (openComplaints && openComplaints.length > 0) {
      ...
      complaint = openComplaints.find((c) => {
        ...
        if (c.description) {
          ...
          if (
            descWords.some(
              (w) => w.length > 4 && text.toLowerCase().includes(w),
            )
          )
            return true;
        }
    ...
    if (complaint) {
      customerKeyword =
        ...
        fallbackExtractCustomerName(complaint.description) ||
        'Customer';
      ...
      if (customerKeyword && customerKeyword.length > 2) {
        const { data: complaints } = await supabase
          ...
          .is('resolved_at', null)
          .or(
            `customer_name.ilike.%${customerKeyword}%,description.ilike.%${customerKeyword}%`,
          )
          .order('reported_at', { ascending: false })
          .limit(1);
    ...
    if (resolvedCustomerName && resolvedCustomerName !== 'Customer') {
      await supabase
        ...
        .is('resolved_at', null)
        .or(
          `customer_name.ilike.%${resolvedCustomerName}%,description.ilike.%${resolvedCustomerName}%`,
        );
    }
