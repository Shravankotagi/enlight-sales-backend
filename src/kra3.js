  ...
  for (const customer of customers) {
    try {
      ...
      const { data: wonDeals } = await supabase
        .from('deals')
        .select(
          'id, total_amount, won_at, created_at, po_number, stage, deal_items(*)',
        )
      ...
      if (existingTask) {
        ...
        if (
          ...
          existingTask.status === 'reorder_expected'
        ) {
          const lastReminder = existingTask.reminder_sent_at
            ? new Date(existingTask.reminder_sent_at)
            : null;
          ...
          const message = buildOrderFrequencyMessage(
            customer,
            lastDeal,
            daysSinceOrder,
            freqDays,
          ...
          console.log(
            `[Order Frequency Followup] Sent reminder #${newCount} to ${spPhone} for ${customer.customer_name}`,
          );
        }
      } else {
        ...
        const { data: newTask } = await supabase
          ...
          .insert({
            ...
            customer_phone:
              customer.customer_phone || lastDeal?.customer_phone || '',
        ...
        await sendTextMessage(spPhone, message);
        console.log(
          `[Order Frequency Followup] Sent initial alert to ${spPhone} for ${customer.customer_name}`,
        );
      }
...
async function checkVisitInterestFollowups(supabase) {
  ...
  for (const task of visitTasks) {
    try {
      ...
      const lastReminder = task.reminder_sent_at
        ? new Date(task.reminder_sent_at)
        : null;
      if (lastReminder) {
      ...
      await sendTextMessage(spPhone, message);
      console.log(
        `[Visit Interest Followup] Sent alert #${newCount} to ${spPhone} for ${task.customer_name}`,
      );
...
function buildOrderFrequencyMessage(
  customer,
  lastDeal,
  daysSinceOrder,
  freqDays,
  ...
  const lastOrderDateStr =
    lastDeal?.won_at || lastDeal?.created_at || customer.last_order_date;
  const formattedLastDate = lastOrderDateStr
    ? new Date(lastOrderDateStr).toLocaleDateString('en-IN')
    : 'Previous Order';
  const lastAmtStr = lastDeal?.total_amount
    ? ` (${formatINR(lastDeal.total_amount)})`
    : '';
...
function buildVisitInterestMessage(task, reminderCount) {
  ...
  const prodMatch =
    notesContext.match(/interest in\s+([^.]+)/i) ||
    notesContext.match(/requirement:\s*([^|\]]+)/i);
  if (prodMatch) {
    productStr = prodMatch[1].trim();
  }

  const visitDateStr = task.created_at
