...
function extractFollowupDays(text, defaultDays = 3) {
  ...
  const m1 = lower.match(
    /(?:follow\s*up|remind(?:\s*me)?|after|in|decide\s*in|within)\s*(?:in|after)?\s*(\d+)\s*(?:days?|din)/i,
  );
  if (m1 && m1[1]) {
  ...
  const m2 = lower.match(
    /\b(\d+)\s*(?:days?|din)\s*(?:baad|later|after|followup|follow\s*up)/i,
  );
  if (m2 && m2[1]) {
...
async function getSalesManagerPhone(salespersonPhone, supabaseClient) {
  ...
  try {
    const { data: emps } = await supabase
      .from('employees')
      .select(
        'id, employee_id, phone, name, role, manager_phone, manager_id, reports_to_employee_id',
      );

    if (!emps || emps.length === 0) return process.env.SALES_LEAD_PHONE || null;

    const rep = emps.find(
      (e) => (e.phone || '').replace(/\D/g, '').slice(-10) === clean,
    );
    if (rep) {
      ...
      if (rep.reports_to_employee_id) {
        const mgr = emps.find(
          (e) => e.employee_id === rep.reports_to_employee_id,
        );
        if (mgr?.phone) return mgr.phone;
      }
    }
    ...
    const activeMgr = emps.find(
      (e) => (e.role === 'sales_manager' || e.role === 'manager') && e.phone,
    );
...
function buildEnrichedFollowupAlert({
  ...
  const cleanDealCode = dealId
    ? String(dealId).replace(/^#?(?:DEAL|INQ)-?/i, '')
    : '';
  const identifierStr = isWon
    ? poNumber
      ? `PO: *${poNumber}*`
      : cleanDealCode
        ? `Inquiry ID: *#INQ-${cleanDealCode}*`
        : ''
    : cleanDealCode
      ? `Inquiry ID: *#INQ-${cleanDealCode}*`
      : '';
  ...
  const repLine =
    recipientRole === 'manager' && salespersonName
...
async function checkRecurringCustomers() {
  ...
  try {
    console.log(
      '[FollowUp Engine] Running configurable follow-up alert checks...',
    );
    await checkOrderFrequencyFollowups(supabase);
    await checkVisitInterestFollowups(supabase);
    console.log(
      '[FollowUp Engine] Follow-up alert checks completed successfully.',
    );
  } catch (error) {
  ...
  for (const customer of customers) {
    try {
      ...
      const { data: wonDeals } = await supabase
        .from('deals')
        .select(
          'id, total_amount, won_at, created_at, po_number, stage, product_name, material_grade, deal_items(*)',
        )
      ...
      const existingTask =
        existingTasks && existingTasks.length > 0 ? existingTasks[0] : null;

      // If resolved after last order, skip
      if (
        existingTask &&
        (existingTask.status === 'resolved' || existingTask.status === 'closed')
      ) {
        const resolvedDate = new Date(
          existingTask.resolved_at || existingTask.created_at,
        );
          ...
          .insert({
            ...
            due_date: new Date(
              lastOrderDate.getTime() +
                (freqDays + spThresholdDays) * 24 * 60 * 60 * 1000,
            ).toISOString(),
            status: 'pending',
      ...
      const product =
        lastDeal?.product_name ||
        lastDeal?.material_grade ||
        'HR / CR Steel Products';
      ...
      if (
        daysOverdue >= spThresholdDays &&
        (!currentTask?.reminder_sent_at ||
          (now - new Date(currentTask.reminder_sent_at)) / (1000 * 60 * 60) >=
            72)
      ) {
        ...
        console.log(
          `[Order Frequency Followup] Sent Salesperson alert (Day 3) to ${spPhone} for ${customer.customer_name}`,
        );
      }

      // 2. Manager Alert: Trigger at 5 days overdue (or every 5 days)
      if (
        daysOverdue >= mgrThresholdDays &&
        (!currentTask?.escalated_at ||
          (now - new Date(currentTask.escalated_at)) / (1000 * 60 * 60) >= 120)
      ) {
        ...
        if (mgrPhone) {
          ...
          console.log(
            `[Order Frequency Followup] Sent Manager alert (Day 5) to ${mgrPhone} for ${customer.customer_name}`,
          );
        }
      }
      ...
      console.error(
        `[Order Frequency Followup] Error for ${customer.customer_name}:`,
        custErr.message,
      );
    }
  }
...
async function checkVisitInterestFollowups(supabase) {
  ...
  for (const task of visitTasks) {
    try {
      ...
      const customDaysMatch = (task.resolution_notes || '').match(
        /decision timeframe:\s*(\d+)/i,
      );
      const spScheduleDays = customDaysMatch
        ? parseInt(customDaysMatch[1], 10)
        : 3;
      ...
      if (recentOrders && recentOrders.length > 0) {
        await supabase
          ...
          .eq('id', task.id);
        console.log(
          `[Visit Interest Followup] Auto-resolved task for ${task.customer_name} (Order received)`,
        );
        continue;
      }
      ...
      const prodMatch =
        (task.resolution_notes || '').match(/interest in\s+([^.]+)/i) ||
        (task.resolution_notes || '').match(/requirement:\s*([^|\]]+)/i);
      ...
      if (
        daysSinceVisit >= spScheduleDays &&
        (!task.reminder_sent_at ||
          (now - new Date(task.reminder_sent_at)) / (1000 * 60 * 60) >=
            spScheduleDays * 24)
      ) {
        const spMsg = buildEnrichedFollowupAlert({
          recipientRole: 'salesperson',
        ...
        console.log(
          `[Visit Interest Followup] Sent Salesperson alert (Day ${spScheduleDays}) to ${spPhone} for ${task.customer_name}`,
        );
      }

      // 2. Manager Alert at X + 2 days (or default 5)
      if (
        daysSinceVisit >= mgrScheduleDays &&
        (!task.escalated_at ||
          (now - new Date(task.escalated_at)) / (1000 * 60 * 60) >=
            mgrScheduleDays * 24)
      ) {
        const mgrPhone = await getSalesManagerPhone(spPhone, supabase);
        if (mgrPhone) {
          ...
          console.log(
            `[Visit Interest Followup] Sent Manager alert (Day ${mgrScheduleDays}) to ${mgrPhone} for ${task.customer_name}`,
          );
        }
      }
      ...
      console.error(
        `[Visit Interest Followup] Error for task ${task.id}:`,
        taskErr.message,
      );
    }
  }
 ...
 */
async function resolveCustomerFollowupTasks(
  customerName,
  senderPhone,
  ...
  try {
    ...
    await query;
    console.log(
      `[FollowUp Engine] Auto-resolved pending follow-up tasks for ${customerName} (${activityType})`,
    );
  } catch (err) {
    console.warn(
      `[FollowUp Engine] Error resolving follow-up tasks for ${customerName}:`,
      err.message,
    );
  }
}
  ...
  try {
    ...
    if (task) {
      ...
      let cleanText = text.replace(
        /^(visited|called|lost|ordered|followed up with|follow up with|followed|followup|follow-up)\s+/i,
        '',
      );
