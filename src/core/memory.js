...
function normalizeMessageObj(item) {
  if (!item) return null;
  const role =
    item.role === 'human' || item.role === 'user' ? 'user' : 'assistant';
  const content =
...
async function getRawChatHistory(senderPhone) {
  ...
  try {
    ...
    if (
      session &&
      session.length > 0 &&
      Array.isArray(session[0].chat_history)
    ) {
...
async function getCustomerThreadHistory(senderPhone, customerName) {
  ...
  customerThreadMap.set(
    threadKey,
    matched.slice(-MAX_CUSTOMER_THREAD_MESSAGES),
  );
...
async function getCustomerFactSheet(customerName, senderPhone) {
  ...
  try {
    ...
    let custQuery = supabase
      ...
      .select(
        'customer_name, customer_phone, customer_gst, customer_address, contact_person, target_frequency_days, tier',
      )
    ...
    if (cust) {
      ...
      factSheet +=
        '\n- Order Frequency: ' +
        (cust.target_frequency_days || 30) +
        ' days | Tier: ' +
        (cust.tier || 'C');
      factSheet +=
        '\n- Missing Profile Fields: ' +
        (missing.length > 0 ? missing.join(', ') : 'None (Complete)');
    }

    if (openDeals && openDeals.length > 0) {
      factSheet += '\n\n- Active Pipeline Inquiries:';
      openDeals.forEach((d) => {
        const cleanCode = (d.deal_code || 'ID').replace(
          /^#?(?:DEAL|INQ)-?/i,
          '',
        );
        factSheet +=
    ...
    console.error(
      '[Memory] Error generating customer fact sheet:',
      err.message,
    );
    return '';
  }
...
async function getActiveContextPrompt(senderPhone) {
  ...
  try {
    ...
    const activeCustomer =
      session?.[0]?.active_customer_name || crossCtx.activeCustomer;
    const lastIntent = session?.[0]?.last_intent || 'general';

    let historySection = '';
    if (crossCtx.formattedHistory) {
      historySection =
        '\n\n## ROLLING CONVERSATION HISTORY (Last ' +
        crossCtx.messages.length +
        ' Messages across all agents):\n' +
    ...
    if (thread.length > 2) {
      threadSection =
        '\n\n## CUSTOMER-SPECIFIC DIALOGUE THREAD (' +
        activeCustomer +
        '):\n' +
        thread
          .map(
    ...
    const activeDealStr = crossCtx.activeDealId
      ? '#' + crossCtx.activeDealId
      : 'None';
    ...
    return (
      ...
      '1. If the salesperson refers to "that inquiry", "the deal", "this customer", "the same customer", "update it", or provides details without naming the customer, resolve it to "' +
      activeCustomer +
      '" and Inquiry ' +
      (crossCtx.activeDealId
        ? '#' + crossCtx.activeDealId
        : 'active in context') +
      '!\n' +
      '2. If profile info (location/city, GST number, mobile phone, contact person/owner) is provided WITHOUT naming a company, attribute it to "' +
      activeCustomer +
      '" and call update_customer_profile.\n' +
      '3. NEVER ask "which company" or treat location/GST replies as order searches when an active customer "' +
      activeCustomer +
      '" is in this context window!\n' +
      '4. If a message specifies a requirement or quantity (e.g. "Need 25 MT", "wants HR Coil", "create deal") WITHOUT repeating the customer name, assume it refers to "' +
      activeCustomer +
      '".\n' +
      '5. If the salesperson replies "Yes", "Confirm", "sahi hai", or gives a PO Number/Inquiry ID to a previous confirmation prompt, associate it with "' +
      activeCustomer +
      '".'
    );
  } catch (err) {
