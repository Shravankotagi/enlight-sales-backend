...
function detectHsnCode(skuText = '', dimensions = '') {
  ...
  if (text.includes('cr sheet') || text.includes('cold rolled'))
    return '72091690';
  if (text.includes('hr coil') || text.includes('hot rolled'))
    return '72083940';
  if (
...
function resolvePlaceOfSupply(address = '') {
  ...
  if (
    ...
    lower.includes('kolhapur') ||
    lower.includes('nagpur') ||
    lower.includes('aurangabad')
  ) {
  ...
  if (lower.includes('haryana') || lower.includes('gurgaon'))
    return 'Haryana (06)';
  if (lower.includes('uttar pradesh') || lower.includes('noida'))
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (
        notoSansPath &&
        notoSansBoldPath &&
        fs.existsSync(notoSansPath) &&
        fs.existsSync(notoSansBoldPath)
      ) {
        try {
          ...
          console.warn(
            'Error registering NotoSans fonts in bot:',
            fErr.message,
          );
        }
      }
      ...
      const customerGstin =
        details.customerGstin || details.customer_gst || details.gstin || '';
      const placeOfSupply = resolvePlaceOfSupply(rawAddress);
      const paymentTerms =
        details.paymentTerms || details.payment_terms || '30 Days Credit';
            ...
            : [
                {
                  ...
                  dimensions:
                    details.dimensions || details.thickness || undefined,
                  quantity: Number(
                    details.quantityTons || details.quantity || 0,
                  ),
                  unit: 'MT',
      ...
      const lineItems = rawLineItems.map((item) => ({
        ...
        hsn_code:
          ...
          item.hsn ||
          detectHsnCode(
            item.sku_text || item.description || '',
            item.dimensions,
          ),
      ...
      const totalTonnageMt = lineItems.reduce(
        (s, i) => s + (i.unit === 'MT' ? i.quantity : 0),
        0,
      );
      const formattedTonnage =
        totalTonnageMt > 0
          ? `${formatIndianCurrency(totalTonnageMt, false)} MT`
      ...
      doc
        ...
        .font(fontRegular)
        .fontSize(8.5)
        .text(formatIndianCurrency(computedSubtotal, true), sumX, summaryY, {
          width: sumW,
          align: 'right',
        });
      ...
      doc
        ...
        .font(fontRegular)
        .fontSize(8.5)
        .text(formatIndianCurrency(cgstVal, true), sumX, summaryY + 14, {
          width: sumW,
          align: 'right',
        });
      ...
      doc
        ...
        .font(fontRegular)
        .fontSize(8.5)
        .text(formatIndianCurrency(sgstVal, true), sumX, summaryY + 28, {
          width: sumW,
          align: 'right',
        });
      ...
      doc
        ...
        .font(fontRegular)
        .fontSize(8.5)
        .text(formatIndianCurrency(roundingVal, true), sumX, summaryY + 42, {
          width: sumW,
          align: 'right',
        });
