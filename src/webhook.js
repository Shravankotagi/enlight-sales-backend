      ...
      if (value.messages && value.messages[0]) {
        ...
        if (isMediaMessage) {
          ...
          if (
            ...
            if (isNewInquiryOrLongMsg) {
              ...
              const numMatch = cleanInput.match(
                /^(?:option\s*|#\s*)?([1-6])\b/i,
              );
              if (numMatch) {
          ...
          if (
            activeSession?.last_intent?.startsWith('pending_delivery_location|')
          ) {
            const parts = activeSession.last_intent.split('|');
            ...
            const extractedLoc =
              extractDeliveryLocation(cleanInput) || cleanInput;
            ...
            if (storedContext) {
              ...
              const reply = await processSalesMessage(
                storedContext.raw_text ||
                  `mark ${customerName} deal as won delivery to ${extractedLoc}`,
                senderPhone,
                storedContext,
              );
              await sendTextMessage(senderPhone, reply);
        ...
        if (
          ...
          const hasValidProductLineItems =
            ...
            extraction.line_items.some(
              (i) =>
                (Number(i.quantity) > 0 || Number(i.quantity_tons) > 0) &&
                (i.sku_text || i.product_name),
            );

          const isChatbotOrQuestion =
            isQuery(raw_text) ||
            /^(show|list|tell|what|how|why|where|can you|give me|is there|which customers|now show|change)\b/i.test(
              raw_text,
            ) ||
            /\b(policy|moq|sop|guideline|portal|login|dashboard)\b/i.test(
              raw_text,
            );

          if (!hasValidProductLineItems || isChatbotOrQuestion) {
            console.log(
              `[Webhook] Skipped saving inquiry for non-product message: "${raw_text.slice(0, 50)}"`,
            );
            return;
          }
