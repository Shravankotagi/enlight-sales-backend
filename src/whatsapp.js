...
async function sendTextMessage(to, message) {
  ...
  if (!formattedMessage) {
    console.warn(
      'Message body was empty after formatting; skipping WhatsApp send.',
    );
    return null;
  }
...
async function sendInteractiveButtons(
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      console.log(
        `Successfully sent WhatsApp interactive buttons to ${to} (attempt ${attempt}). Message ID: ${response.data.messages[0].id}`,
      );
      return response.data;
    } catch (error) {
      console.error(
        `WhatsApp interactive buttons send error (attempt ${attempt}/3):`,
        error.message,
      );
      if (attempt === 3) {
        console.error(
          'FULL INTERACTIVE SEND ERROR details:',
          JSON.stringify(error.response?.data || error.message, null, 2),
        );
        if (!textToSendFirst) {
...
async function sendInteractiveList(
    ...
    for (const r of s.rows || []) {
      ...
      rows.push({
        ...
        description: r.description
          ? String(r.description).trim().slice(0, 72)
          : undefined,
      });
      totalRows++;
    }
  ...
  const interactivePayload = {
    ...
    action: {
      button: String(buttonText || 'Choose Action')
        .trim()
        .slice(0, 20),
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      console.log(
        `Successfully sent WhatsApp interactive list to ${to} (attempt ${attempt}). Message ID: ${response.data.messages[0].id}`,
      );
      return response.data;
    } catch (error) {
      console.error(
        `WhatsApp interactive list send error (attempt ${attempt}/3):`,
        error.message,
      );
      if (attempt === 3) {
        console.error(
          'FULL LIST SEND ERROR details:',
          JSON.stringify(error.response?.data || error.message, null, 2),
        );
        return await sendTextMessage(to, formattedBody);
      } else {
