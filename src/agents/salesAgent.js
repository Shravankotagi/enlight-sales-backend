...
async function processSalesMessage(text, senderPhone, overrideData = null) {
  try {
    ...
    for (const item of rawItems) {
      ...
      const qty =
        Number(item.quantity_mt || item.quantity || item.qty || 0) || 0;
      let rate =
        Number(item.rate_per_mt || item.rate || item.unitPrice || 0) || 0;
      const rawDim =
        item.dimensions ||
    ...
    if (hasUnlistedMaterial && calculatedTotal === 0) {
      ...
      return (
        `⚠️ *Product Price Confirmation Required*\n\n` +
    ...
    if (processedItems.length > 0) {
      itemsBreakdownStr = processedItems
        .map((pi) => {
          ...
          const rateStr =
            pi.rate > 0
              ? ` @ ₹${Number(pi.rate).toLocaleString('en-IN')}/MT`
              : '';
          const amtStr =
            pi.itemAmount > 0
