...
async function upsertAccount(name, profile, salespersonName, token) {
  ...
  try {
    const searchRes = await axios.get(`${ZOHO_BIGIN_BASE}/Accounts/search`, {
      headers: zohoHeaders(token),
      params: {
        criteria: `(Account_Name:equals:${encodeURIComponent(cleanName)})`,
      },
    });
  ...
  try {
    ...
    console.error(
      '[BiginSync] Account create error:',
      err.response?.data || err.message,
    );
    return null;
  }
...
async function upsertContact(profile, salespersonName, token) {
  ...
  try {
    const searchRes = await axios.get(`${ZOHO_BIGIN_BASE}/Contacts/search`, {
      headers: zohoHeaders(token),
      params: {
        criteria: `(Last_Name:equals:${encodeURIComponent(lastName)})`,
      },
    });
  ...
  if (existingId) {
    try {
      ...
      console.log(
        `[BiginSync] Contact updated: ${personName} -> ${companyName} (${existingId})`,
      );
      return existingId;
    } catch (err) {
      return existingId;
  ...
  try {
    ...
    if (newId) {
      console.log(
        `[BiginSync] Contact created: ${personName} -> ${companyName} (${newId})`,
      );
    }
    return newId;
  } catch (err) {
...
async function upsertDeal(
  ...
  const primaryItem =
    dealItems && dealItems[0] && dealItems[0].sku_text
      ...
      : 'Metal Deal';
  const shortId = dbDealId
    ? ` [#${dbDealId.substring(0, 6).toUpperCase()}]`
    : '';
  ...
  try {
    ...
    validAccountId = await upsertAccount(
      name,
      custProfile,
      salespersonName,
      token,
    ...
    if (
      !validContactId ||
      typeof validContactId !== 'string' ||
      validContactId.length < 5
    ) {
...
async function syncActivity(activityType, data) {
  setImmediate(async () => {
    ...
    if (
      ...
      console.log(
        '[BiginSync] Zoho sync is PAUSED / Read-only mode active - skipping outbound Zoho Bigin sync',
      );
    ...
    try {
      ...
      switch (normalizedType) {
        ...
        case 'deal_stage': {
          ...
          const existingDeal = await findDeal(
            customerName,
            data.biginDealId,
            token,
          );
          if (existingDeal) {
            try {
              ...
              if (data.dealId) {
                ...
                await sb
                  .from('deals')
                  .update({ bigin_deal_id: existingDeal.id })
                  .eq('id', data.dealId);
              }
            } catch (err) {
              console.error(
                '[BiginSync] Stage update error:',
                err.response?.data || err.message,
              );
            }
          } else {
