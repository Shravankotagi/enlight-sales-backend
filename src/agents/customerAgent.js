...
async function getMonthlyOnboardCount(senderPhone) {
  ...
  const distinct = new Set(
    data.map((r) => (r.customer_name || '').toLowerCase().trim()),
  );
...
async function processCustomerMessage(text, senderPhone) {
  try {
    ...
    if (
      fullSession &&
      fullSession.last_intent &&
      fullSession.last_intent.startsWith('duplicate_check|')
    ) {
      const cleanInput = text.trim().toLowerCase();
      let payload = null;
      try {
        payload = JSON.parse(
          fullSession.last_intent.replace('duplicate_check|', ''),
        );
      } catch (e) {
        /* ignore parse error */
      }
      ...
      if (payload) {
        const isYes =
          ...
          cleanInput.startsWith('yes') ||
          cleanInput.startsWith('ha ') ||
          cleanInput.startsWith('haa');
        ...
        const isNo =
          ...
          cleanInput.startsWith('no') ||
          cleanInput.startsWith('nahi');

        if (isYes) {
          // User confirmed YES: Link existing company to this salesperson's portfolio
          await saveActiveSession(
            senderPhone,
            payload.existingCustomerName,
            'duplicate_confirmed',
          );
          ...
          if (payload.newPayload?.phone)
            updateData.customer_phone = payload.newPayload.phone;
          if (payload.newPayload?.gst)
            updateData.customer_gst = payload.newPayload.gst;
          if (payload.newPayload?.city)
            updateData.customer_address = payload.newPayload.city;
          if (payload.newPayload?.contact_person)
            updateData.contact_person = payload.newPayload.contact_person;
          if (payload.newPayload?.order_frequency_days)
            updateData.avg_order_frequency_days = Number(
              payload.newPayload.order_frequency_days,
            );
          ...
          // Log KRA 2
          const alreadyLogged = await isKRA2AlreadyLogged(
            senderPhone,
            payload.existingCustomerName,
          );
          if (!alreadyLogged) {
    ...
    if (!data)
      throw new Error(
        'Could not parse customer onboarding JSON from LLM response',
      );
    ...
    if (
      !data.customer_name &&
      activeCustomer &&
      activeCustomer !== 'PENDING_PROFILE' &&
      activeCustomer !== 'PENDING_CORRECT_NAME' &&
    ...
    if (
      !data.customer_name ||
      data.customer_name.trim().toLowerCase() === 'unknown'
    ) {
      if (
        data.phone ||
        data.contact_person ||
    ...
    if (exactMatch) {
      ...
      if (
        matchRepClean === senderCleanPhone ||
        (!matchRepClean && !scope.isAdmin)
      ) {
        const updateFields = {
          ...
          contact_person:
            data.contact_person || exactMatch.contact_person || null,
          is_active: true,
          updated_at: new Date().toISOString(),
        };
        if (data.order_frequency_days) {
          updateFields.avg_order_frequency_days = Number(
            data.order_frequency_days,
          );
        }
