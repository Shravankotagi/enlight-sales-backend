...
function extractProductFromText(text) {
  ...
  const m2 = str.match(
    /\b(MS\s+Plate|MS\s+Plates|MS\s+Sheet|MS\s+Sheets|HR\s+Coil|HR\s+Coils|HR\s+Sheet|HR\s+Sheets|CR\s+Coil|CR\s+Coils|CR\s+Sheet|CR\s+Sheets|TMT\s+Bar|TMT\s+Bars|GI\s+Sheet|GI\s+Sheets|GI\s+Coil|GI\s+Coils|Chequered\s+Plate|Chequered\s+Plates|MS\s+Pipe|MS\s+Pipes)\b/i,
  );
  if (m2) {
    return m2[1];
  }
 ...
 */
async function resolveProductFromContext(
  dealId,
  poNumber,
  customerName,
  affectedProduct,
  ...
  if (poNumber) {
    try {
      ...
      if (
        poDeals &&
        poDeals.length > 0 &&
        poDeals[0].deal_items &&
        poDeals[0].deal_items.length > 0
      ) {
  ...
  if (customerName) {
    try {
      ...
      console.warn(
        'Error resolving product from customer won deals:',
        e.message,
      );
    }
  }
...
async function processSingleComplaint(data, originalText, senderPhone) {
  ...
  if (
    activeSessionObj &&
    activeSessionObj.last_intent &&
    activeSessionObj.last_intent.startsWith('complaint_confirm_deal|')
  ) {
    const rawPayload = activeSessionObj.last_intent.replace(
      'complaint_confirm_deal|',
      '',
    );
    try {
      const draft = JSON.parse(rawPayload);
      const isAffirmative =
        /^(yes|haan|confirm|correct|right|sahi|sahi hai|yep|yup|ok|okay|ha|bilkul)$/i.test(
          originalText.trim(),
        ) || Boolean(data.is_confirmation);
      if (isAffirmative) {
        if (!data.customer_name && draft.customer_name)
          data.customer_name = draft.customer_name;
        if (!data.deal_id && draft.dealId) data.deal_id = draft.dealId;
        if (!data.po_number && draft.poNumber) data.po_number = draft.poNumber;
        if (!data.affected_product && draft.product)
          data.affected_product = draft.product;
        if (
          (!data.description || data.description === originalText) &&
          draft.description
        )
          data.description = draft.description;
        if (
          (!data.complaint_type || data.complaint_type === 'other') &&
          draft.complaintType
        )
          data.complaint_type = draft.complaintType;
  ...
  if (!data.customer_name) {
    ...
    if (
      activeCustomer &&
      activeCustomer.toLowerCase() !== 'unknown' &&
      activeCustomer.toLowerCase() !== 'null'
    ) {
  ...
  if (!data.customer_name) {
    ...
    if (dealIdMatch) {
      ...
      const { data: matchedDeals } = await supabase
        ...
        .limit(100);
      const found = (matchedDeals || []).find((d) =>
        d.id.toLowerCase().startsWith(shortCode),
      );
      if (found) {
        data.customer_name = found.customer_name;
        data.deal_id = found.id;
        if (found.po_number && !data.po_number)
          data.po_number = found.po_number;
      }
    }
  }
  ...
  if (data.action === 'resolve') {
    const openComplaint = await getOpenComplaint(
      finalCustomerName,
      senderPhone,
      data.deal_id,
    );
    ...
    const isGenericResolveText =
      /^(resolved|resolve|issue sorted|fixed|done|ho gaya|settled)$/i.test(
        resolutionNotes,
      );
    if (!resolutionNotes || isGenericResolveText) {
      return (
        `ℹ️ *Resolution Notes Required for ${finalCustomerName}*\n\n` +
    ...
    if (openComplaint) {
      ...
      const reportedAt = new Date(
        openComplaint.created_at || openComplaint.reported_at || Date.now(),
      );
      const resolutionTimeHrs = Math.max(
        1,
        Math.round(
      ...
      const existingProd =
        openComplaint.product_name || openComplaint.affected_product;
      const isGeneric =
        !existingProd ||
        [
          'general material',
          ...
          'steel',
          'null',
        ].includes(existingProd.toLowerCase().trim());
      ...
      if (
        isGeneric &&
        updatedProduct &&
        updatedProduct !== 'General Material'
      ) {
      ...
      const alreadyLogged = await isKRA8AlreadyLogged(
        senderPhone,
        finalCustomerName,
      );
      if (!alreadyLogged) {
      ...
      const orderRef = openComplaint.po_number
        ...
        : openComplaint.deal_id
          ? `Inquiry: *#INQ-${openComplaint.deal_id.substring(0, 6).toUpperCase()}*`
          : '';
      ...
      try {
        ...
        console.warn(
          '[ComplaintAgent] Follow-up auto-resolution notice:',
          rErr.message,
        );
      }
  ...
  if (!targetDealId && !targetPoNumber) {
    ...
    if (activeDeals.length === 1 && !data.is_confirmation) {
      ...
      await saveActiveSession(
        senderPhone,
        finalCustomerName,
        `complaint_confirm_deal|${draftPayload}`,
      );
  ...
  try {
    const cleanCodeForLog = targetDealId
      ? targetDealId.startsWith('DEAL-') || targetDealId.startsWith('INQ-')
...
async function processComplaintMessage(text, senderPhone) {
  try {
    ...
    if (!data)
      throw new Error('Could not parse complaint JSON from LLM response');
