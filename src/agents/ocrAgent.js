...
async function processSalesImage(
  ...
  try {
    ...
    const rawCustName =
      extraction.customer && extraction.customer.name
        ? String(extraction.customer.name).trim()
        : '';
    const isGenericName =
      !rawCustName ||
      rawCustName === 'null' ||
      rawCustName === 'None' ||
      rawCustName.length < 2;
    const custName = isGenericName ? null : rawCustName;
    const officialCustomerName = custName
      ? await verifyAndGetCustomerName(custName, senderPhone)
      : null;
    ...
    if (isPo) {
      ...
      try {
        ...
        console.log(
          '[OCRAgent] Calling backend process-po for PO:',
          poNumber,
          'customer:',
          finalCustomerName,
        );
        ...
        console.log(
          '[OCRAgent] Backend process-po-internal success, dealId:',
          dealId,
        );
      } catch (backendErr) {
        console.error(
          '[OCRAgent] Backend process-po-internal failed, falling back to direct Supabase:',
          backendErr.message,
        );
        ...
        const { data: openDeals, error: openDealsErr } =
          await openDealsQuery.limit(1);

        if (openDealsErr) {
          console.error(
            '[OCRAgent] openDeals query error:',
        ...
        if (openDeals && openDeals.length > 0) {
          ...
          if (updateErr) {
            console.error(
              '[OCRAgent] Fallback deal update error:',
              updateErr.message,
            );
          } else {
            console.log(
              '[OCRAgent] Fallback: updated existing deal to won, dealId:',
              dealId,
            );
          }
        } else {
          ...
          if (insertErr) {
            console.error(
              '[OCRAgent] Fallback deal insert error:',
              insertErr.message,
              insertErr,
            );
          } else {
            ...
            console.log(
              '[OCRAgent] Fallback: created new won deal, dealId:',
              dealId,
            );
          }
        }
      ...
      if (dealErr) {
        console.error(
          '[OCRAgent] Error inserting inquiry deal:',
    ...
    try {
      ...
      console.warn(
        '[OCRAgent] Non-blocking activity log notice:',
        actErr?.message,
      );
    }
    ...
    if (extraction.line_items && extraction.line_items.length > 0) {
      itemsBreakdown = extraction.line_items
        .map((i) => {
          ...
          const rateStr = i.rate
            ? ` @ ₹${Number(i.rate).toLocaleString('en-IN')}/${unit}`
            : '';
