...
async function generateFullKRAReport(
  senderPhoneOrScope,
  customMonthRange = null,
) {
  ...
  try {
    let scope =
      typeof senderPhoneOrScope === 'object' &&
      senderPhoneOrScope !== null &&
      senderPhoneOrScope.role
        ? senderPhoneOrScope
        : await getAccessibleSalespersonPhonesForBot(
            typeof senderPhoneOrScope === 'string' ? senderPhoneOrScope : '',
          );

    // Helper to apply phone filtering based on role
    function applyPhoneFilter(query, phoneField = 'salesperson_phone') {
      if (scope.isAdmin || !scope.phones) {
        return query; // Admin sees all records
      }
      if (Array.isArray(scope.phones) && scope.phones.length > 0) {
        return query.in(phoneField, scope.phones);
      }
      return query.eq(phoneField, scope.phone || '0000000000');
    }
    ...
    let dealsQuery = supabase
      ...
      .lte('created_at', end);
    dealsQuery = applyPhoneFilter(dealsQuery, 'salesperson_phone');
    ...
    let inquiriesQuery = supabase
      ...
      .gte('created_at', start)
      .lte('created_at', end);
    inquiriesQuery = applyPhoneFilter(inquiriesQuery, 'salesperson_phone');
    ...
    let kraLogsQuery = supabase
      ...
      .gte('created_at', start)
      .lte('created_at', end);
    kraLogsQuery = applyPhoneFilter(kraLogsQuery, 'salesperson_phone');
    ...
    let visitsQuery = supabase
      ...
      .lte('visited_at', end);
    visitsQuery = applyPhoneFilter(visitsQuery, 'salesperson_phone');
    ...
    let complaintsQuery = supabase
      ...
      .lte('reported_at', end);
    complaintsQuery = applyPhoneFilter(complaintsQuery, 'reported_by');
    ...
    let paymentsQuery = supabase
      ...
      .gte('created_at', start)
      .lte('created_at', end);
    paymentsQuery = applyPhoneFilter(paymentsQuery, 'salesperson_phone');

    let recurringQuery = supabase
      .from('recurring_customers')
      .select('*')
      .eq('is_active', true);
    recurringQuery = applyPhoneFilter(
      recurringQuery,
    ...
    const conversionRate =
      totalInquiries > 0 ? Math.round((wonDeals / totalInquiries) * 100) : 0;
    ...
    const crmCompliance = Math.min(
      100,
      Math.round((daysWithActivity / workingDays) * 100),
    );

    // KRA 7 - Zero Rejection
    const rejections = kraLogs.filter((l) => l.kra_number === 7).length;
    ...
    const report =
      ...
      `${newCustomers >= 3 ? ' Target met!' : ` ${3 - newCustomers} more needed`}\n\n` +
      `*3 Customer Retention & Order Frequency*\n` +
      `Recurring customers: ${recurring.length}\n` +
      `Ordered this month: ${recurringWithOrder}\n` +
      `${retentionRate >= 80 ? '' : ''} Retention: ${retentionRate}%\n\n` +
      `*4 Enquiry & Pipeline Conversion*\n` +
      `Inquiries: ${totalInquiries} | Won: ${wonDeals}\n` +
      `${conversionRate >= 70 ? '' : ''} Rate: ${conversionRate}%` +
      ` (target: 70-80%)\n\n` +
      `*5 Payment Collection & Outstanding*\n` +
      `Collected: ${collectedPayments.length}\n` +
      `Pending: ${pendingPayments.length}\n` +
      ` Overdue: ${overduePayments.length}\n` +
      `Outstanding: ${formatINR(totalOutstanding)}\n\n` +
      `*6 CRM & Zoho Bigin Sync*\n` +
      `Active days: ${daysWithActivity}/${workingDays}\n` +
      `${crmCompliance >= 90 ? '' : ''} Compliance: ${crmCompliance}%\n\n` +
      `*7 Order Accuracy & Zero Rejection*\n` +
      `${rejections === 0 ? ' Zero rejections!' : ` ${rejections} rejection(s) logged`}\n\n` +
