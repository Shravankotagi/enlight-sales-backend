  ...
  if (parsed.customer && typeof parsed.customer === 'object') {
    ...
    if (name) {
      ...
      const addressPrefixRegex =
        /^(Akshar Business Park|Business Park|Office No|Plot No|Sector|Industrial Area|MIDC|Gat No|Survey No|Phase)[,\s\-]+/i;
      name = name.replace(addressPrefixRegex, '').trim();

      const splitOnAddress = name.split(
        /(?:,?\s*(?:Akshar Business Park|Office No|Plot No|Sector \d+|Janta Market|Opp\.|Turbhe|Navi Mumbai|Maharashtra|State Code|PIN|Gat No))/i,
      );
      if (
        splitOnAddress &&
        splitOnAddress[0] &&
        splitOnAddress[0].trim().length > 3
      ) {
...
Return ONLY a valid JSON object. No markdown, no prose, no backticks.`,
    ...
    throw (
      lastError || new Error('Could not parse JSON from Gemini vision response')
    );
  } catch (error) {
